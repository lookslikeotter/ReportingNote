...
//   1) 사건 노트의 `취재가치`를 기준으로 tags의 `사건`·`특종`을 맞춘다 (다른 태그는 그대로).
...
if (fs.existsSync(BOARD)) {
  ...
  for (let i = 0; i < L.length; i++) {
    ...
    for (let j = i + 1; j < L.length; j++) {
      const c = L[j].match(/^\t{1,2}- \[( |x|X)\] (☕ 일상|📰 사건|🔥 특종)\s*$/); if (!c) break
...
for (const f of files) {
  ...
  ;(byDay[+m[1]] ??= {})[name] = want.includes("이벤트") ? "bn-lv-event" : GRADE_SPAN[grade]
}
let rowFixes = 0
