import { QuartzComponent, QuartzComponentConstructor, QuartzComponentProps } from "./types"
// @ts-ignore
import script from "./scripts/liveUpdate.inline"
import { classNames } from "../util/lang"

// 봉누도2 — 새로고침 버튼 (경로 표시 왼쪽). 원본은 볼트의 .quartz/quartz/components/RefreshButton.tsx (빌드 때 Quartz에 복사).
// 누르면 지금 페이지와 사이트 스크립트·스타일·데이터를 서버에서 새로 받은 뒤 다시 불러온다 (Ctrl+F5와 비슷).
// 함께 싣는 liveUpdate.inline.ts가 새 배포를 알아서 확인해 자동으로 갱신하기도 한다.

const RefreshButton: QuartzComponent = ({ displayClass }: QuartzComponentProps) => (
  <button
    class={classNames(displayClass, "bn-refresh")}
    aria-label="새로고침"
    title="새로고침 (사이트 새 버전 받기)"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="18"
      height="18"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      stroke-width="2"
      stroke-linecap="round"
      stroke-linejoin="round"
    >
      <path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8" />
      <path d="M21 3v5h-5" />
    </svg>
  </button>
)

RefreshButton.afterDOMLoaded = script

export default (() => RefreshButton) satisfies QuartzComponentConstructor
