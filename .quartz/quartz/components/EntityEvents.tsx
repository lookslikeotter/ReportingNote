...
// 봉누도2 — 인물·세력·장소 페이지 아래 '사건 기록' 표. 원본은 볼트의 .quartz/quartz/components/EntityEvents.tsx (빌드 때 Quartz에 복사).
// 일지 표에서 이 인물·세력이 '관련 인물' 칸에 있는 행(명총희는 '입수 경로' 칸이 인물 링크인 행도)과
// 사건의 `가담인물`에 이 인물이 있는 행을 일차 순으로 모아 일지와 같은 모양의 표로 보여 준다. ☕도 들어간다.
// 장소 페이지는 '장소' 칸이 이 장소(링크, 또는 이름·별칭·포함장소와 같은 글자)인 행.
// 여기서는 자리만 만들고, 표는 entityEvents.inline.ts가 페이지를 연 뒤 일지 표를 읽어 채운다.

// 인물·세력·장소 페이지 (목록 노트 제외)
const showOn = (slug: string) =>
  /^0[234]-(인물|세력|장소)\//.test(slug) && !/(인물-목록|세력-목록|장소-목록)$/.test(slug)
